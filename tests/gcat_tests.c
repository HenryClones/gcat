...
static int gcat_test01()
{
    ...
    uint32_t *w = (uint32_t *) bounds_checked_access(data1, 1, 1, 64);
    if (w != NULL)
    {
        return 1;
    }
    uint32_t *x = (uint32_t *) bounds_checked_access(data1, 1, 1, sizeof(uint32_t));
    *x = 30;
    uint8_t *data2 = gall(40, NULL);
    uint32_t *y = (uint32_t *) bounds_checked_access(data2, 1, 1, sizeof(uint32_t));
    *y = 24;
    if (x == y || *x == *y)
    {
        return 1;
    }
    uint32_t *z = (uint32_t *) bounds_checked_access(data1, 1, 1, sizeof(uint32_t));
    if (x != z || *x != *z)
    {
        return 1;
    }
